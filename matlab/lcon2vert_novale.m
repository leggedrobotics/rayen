...
%
 ...
 %%Reduce the equality constraints
  if equalityConstrained && nargout>2
 
        %Extract a linearly independent set of columns of a given matrix [Aeq,beq]
   ...
   if equalityConstrained
       ...
       %null() returns an orthonormal basis for the null space of a matrix.
       Neq=null(Aeq);   
       ...
       elseif isempty(Neq) %This happens when the null space is {0} (i.e., when Aeq is invertible)
           ...
           if inequalityConstrained && ~all(A*x0<=b)
              ...
              return;              
               ...
               %An example of this is three planes interesecting in one point in the middle of a polytope
