...
class WrapperWalkerForImages(nn.Module):
    def __init__(self, A_np, b_np, num_steps):
        super().__init__()
        dim = A_np.shape[1]
        mapper=nn.Sequential(nn.Linear(dim, getNumelInputWalker(dim, num_steps)))
        self.constraint = LinearConstraintWalker(A_np,b_np,num_steps, mapper=mapper, use_max_ellipsoid=False)
